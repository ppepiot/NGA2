...
module simulation
   ...
   real(WP), dimension(:,:,:), allocatable :: IBvf

   !> Problem definition
   real(WP), dimension(3) :: center
   real(WP) :: radius
   real(WP), dimension(3) :: center_fixed
   real(WP) :: radius_fixed
...
contains
   ...
   function levelset_fixed_drop(xyz,t) result(G)
      implicit none
      real(WP), dimension(3),intent(in) :: xyz
      real(WP), intent(in) :: t
      real(WP) :: G
      G=radius_fixed-sqrt(sum((xyz-center_fixed)**2))
   end function levelset_fixed_drop
   ...
   subroutine simulation_init
      ...
      allocate_work_arrays: block
         allocate(IBvf(cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_))
      ...
      create_and_initialize_vof: block
         ...
         center=[0.001_WP,0.075_WP,0.0_WP]
         radius=0.02_WP
            ...
            do j=vf%cfg%jmino_,vf%cfg%jmaxo_
               do i=vf%cfg%imino_,vf%cfg%imaxo_
                  ...
                  do sj=0,3
                     do si=0,10
                        vol=0.0_WP
                        radius_fixed=0.005_WP
                        if (mod(sj,2).eq.0) then
                           center_fixed=[-0.05_WP+1.25_WP*radius_fixed+real(si)*2.5_WP*radius_fixed,1.25_WP*radius_fixed+real(sj)*2.5_WP*radius_fixed,0.0_WP]
                        else
                           center_fixed=[-0.05_WP+real(si)*2.5_WP*radius_fixed,1.25_WP*radius_fixed+real(sj)*2.5_WP*radius_fixed,0.0_WP]
                        end if
                        call cube_refine_vol(cube_vertex,vol,area,v_cent,a_cent,levelset_fixed_drop,0.0_WP,1)
                        IBvf(i,j,k)=IBvf(i,j,k)-vol/vf%cfg%vol(i,j,k)
                     end do
                  end do
               end do
            end do
      ...
      create_ensight: block
         ...
         call ens_out%add_scalar('IBVF',IBvf)
               ...
               do k=fs%cfg%kmin_,fs%cfg%kmax_
                  do j=fs%cfg%jmin_,fs%cfg%jmax_
                     do i=fs%cfg%imin_,fs%cfg%imax_
                        fs%U(i,j,k)=fs%U(i,j,k)*sum(fs%itpr_x(:,i,j,k)*IBvf(i-1:i,j,k))
                        fs%V(i,j,k)=fs%V(i,j,k)*sum(fs%itpr_y(:,i,j,k)*IBvf(i,j-1:j,k))
                        fs%W(i,j,k)=fs%W(i,j,k)*sum(fs%itpr_z(:,i,j,k)*IBvf(i,j,k-1:k))
                     end do
                  end do
               end do
   ...
   subroutine simulation_final
      ...
      deallocate(IBvf,resU,resV,resW,Ui,Vi,Wi)
