...
contains
   ...
   subroutine simulation_init
      ...
      ! Create a one-sided scalar solver
      create_scalar: block
         use hypre_str_class, only: gmres_pfmg2,pcg_pfmg2,DDADIk
         use param, only: param_read
         use scalar_class, only: bcond,dirichlet,neumann,upwind,quick
         type(bcond), pointer :: mybc
         integer :: i,j,k,n
         ! Create scalar solver
         sc=scalar(cfg=cfg,scheme=upwind,name='scalar')
         ! Boundary conditinos
         call sc%add_bcond(name='xm',type=dirichlet,locator=xm_locator_sc,dir='-x')
         call sc%add_bcond(name='xp',type=neumann,  locator=xp_locator   ,dir='+x')
         sc%rho=rho_g
         sc%diff=alpha_g*rho_g
         ! Initialize the linear solver
         ss=ddadi(cfg=cfg,name='Scalar',nst=7)
         ...
         do i=sc%cfg%imin_,sc%cfg%imax_
            if (i.le.i_itf) then
               do j=sc%cfg%jmin_,sc%cfg%jmax_
                  do k=sc%cfg%kmin_,sc%cfg%kmax_
                     sc%SC(i,j,k)=T_w+(T_sat-T_w)/sc%cfg%xm(i_itf)*sc%cfg%xm(i)
                  end do
               end do
            else
               sc%SC(i,:,:)=T_sat
         ...
         do n=1,mybc%itr%no_
            i=mybc%itr%map(1,n); j=mybc%itr%map(2,n); k=mybc%itr%map(3,n)
            sc%SC(i,j,k)=2.0_WP*T_w-sc%SC(i+1,j,k)
         end do
      end block create_scalar
      ...
      create_ensight: block
         ...
         call ens_out%add_scalar('Tg',sc%SC(:,:,:))
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
               ...
               do while (timeSC%it.le.timeSC%itmax)
                  ...
                  do k=cfg%kmin_,cfg%kmax_
                     do j=cfg%jmin_,cfg%jmax_
                        do i=cfg%imin_,cfg%imax_
                           if (i.lt.i_itf) then
                              sc%SC(i,j,k)=0.5_WP*(sc%SC(i,j,k)+sc%SCold(i,j,k))
                           else
                              sc%SC(i,j,k)=T_sat
                           end if
                        end do
                  ...
                  do k=cfg%kmin_,cfg%kmax_
                     do j=cfg%jmin_,cfg%jmax_
                        do i=cfg%imin_,cfg%imax_
                           if (i.lt.i_itf) then
                              resSC(i,j,k)=2.0_WP*rho_g*(sc%SCold(i,j,k)-sc%SC(i,j,k))+timeSC%dt*resSC(i,j,k)
                           else
                              resSC(i,j,k)=0.0_WP
                           end if
                        end do
                  ...
                  call sc%solve_implicit(dt=timeSC%dt,resSC=resSC,rhoU=resU,rhoV=resV,rhoW=resW,i_itf=i_itf)
                  print*,'line 675'
                  ! Apply the residuals
                  ...
                  do k=cfg%kmin_,cfg%kmax_
                     do j=cfg%jmin_,cfg%jmax_
                        do i=cfg%imin_,cfg%imax_
                           if (i.ge.i_itf) sc%SC(i,j,k)=T_sat
                        end do
                     end do
                  end do
         ...
         !             do i=cfg%imin_,cfg%imax_
         !                if (i.lt.i_itf) then
         !                   resSC(i,j,k)=timeSC%dt*resSC(i,j,k)
         !                else
         !                   resSC(i,j,k)=0.0_WP
         !                end if
         !             end do
         ...
         !       print*,'dx = ',cfg%dx(1)
         !       print*,'dt = ',timeSC%dt
         !       print*,'alpha = ',sc%diff(1,1,1)
         !       print*,'D = ',sc%diff(1,1,1)*timeSC%dt*cfg%dxi(1)**2
         !       print*,'RHS = ',resSC(1:i_itf,1,1)
         !       call sc%solve_implicit(dt=timeSC%dt,resSC=resSC,rhoU=resU,rhoV=resV,rhoW=resW,i_itf=i_itf)

         !       print*,'resSC = ',resSC(1:i_itf,1,1)
