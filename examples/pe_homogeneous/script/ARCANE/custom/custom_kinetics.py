...
def print_fortran(
    mechanism,
    f90_filename="custom_kinetics.f90",
    species_qss_names=[],
    use="Cantera",
    routine_name="customkinetics",
    semi_implicit=False,
    rrate=True,
    force=True,
    exponential=False,
    implicit_exponential=False,
    classical=True,
    dt_exp_user=1e-10,
    supplement_dict=None,
    maintain_given_order=True,
    fit_order=0,
    ...
    if use == "NTMIX":
        if exponential and implicit_exponential:
            logger.error(
                "exponential and implicit_exponential options cannot be used at the same time."
            )
        ...
        if implicit_exponential:
            classical = True
            logger.error(
                "Classical chemistry used by default with implicit exponential"
            )
    ...
    if nqss > 0:
        ...
        qss_not_found = [
            spec for spec in species_qss_names if spec not in species_names
        ]
        if qss_not_found:
        ...
        new_ctmech, new_species_qss_names = problematic_qss(
            mechanism, species_qss_names, remove_from=remove_from
        )

        if new_species_qss_names != species_qss_names:
            species_qss_names = new_species_qss_names
            mechanism = mechanisms.Mechanism(
                new_ctmech, parent=mechanism.parent, how=mechanism.how
            )
    ...
    print("Number of thirdbodies nTB = ", nTB)
    print(Plogindex)
    if Plogindex:
        Plog_flag = True
    else:
    ...
    mech_variables["non_qss_species"] = [
        spec for spec in species_names if spec not in species_qss_names
    ]
    mech_variables["non_qss_species_og"] = [
        spec
        for spec in species_names_original
        if spec not in species_qss_names_original
    ]
    ...
    for iforw in range(nr):
        ...
        if orders:
            reaction_order = sum(
                [
                    orders[spec]
                    for spec in ctmech.reaction(reac_label[iforw] - 1).reactants
                ]
            )
        else:
        ...
        if reac_type[iforw] == 4:
            ...
            forward_arrh_inf[0, iFO] = (
                multip * my_arrhenius_inf.pre_exponential_factor / A_correction_factor
            )
            forward_arrh_inf[1, iFO] = my_arrhenius_inf.temperature_exponent
            forward_arrh_inf[2, iFO] = (
                my_arrhenius_inf.activation_energy / E_correction_factor
            )

            # Store Arrhenius parameters for forward, fall-off reaction, converted to SI - 0
            forward_arrh_0[0, iFO] = (
                multip
                * my_arrhenius_0.pre_exponential_factor
                / A_with_M_correction_factor
            )
            forward_arrh_0[1, iFO] = my_arrhenius_0.temperature_exponent
            forward_arrh_0[2, iFO] = (
                my_arrhenius_0.activation_energy / E_correction_factor
            )
            ...
            if is_reversible[iforw] == 1:
                # INF
                reverse_arrh_inf[:, iFO] = reverse_arrhenius_fit(
                    constants,
                    mech_variables,
                    nu[:, iforw],
                    forward_arrh_inf[:, iFO],
                    fit_order=fit_order,
                ...
                # 0
                reverse_arrh_0[:, iFO] = reverse_arrhenius_fit(
                    constants,
                    mech_variables,
                    nu[:, iforw],
                    forward_arrh_0[:, iFO],
            ...
            for index, arr_set in enumerate(my_arrhenius):
                ...
                mylist.append(
                    arr_set[1].pre_exponential_factor * multip
                )  # / A_correction_factor)
            ...
            if reac_type[iforw] == 2:
                ...
                forward_arrh[0, iforw] = (
                    multip
                    * my_arrhenius.pre_exponential_factor
                    / A_with_M_correction_factor
                )
            else:
                # Order of reaction is what it is
                forward_arrh[0, iforw] = (
                    multip * my_arrhenius.pre_exponential_factor / A_correction_factor
                )

            forward_arrh[1, iforw] = my_arrhenius.temperature_exponent
            forward_arrh[2, iforw] = (
                my_arrhenius.activation_energy / E_correction_factor
            )

            # -------------------------------------------
            # Store Arrhenius parameters for reverse, non-fall-off reaction, converted to SI
            if is_reversible[iforw] == 1:
                reverse_arrh[:, iforw] = reverse_arrhenius_fit(
                    constants,
                    mech_variables,
                    nu[:, iforw],
                    forward_arrh[:, iforw],
    ...
    #     precision = 'WP'
        ...
        logger.warning(
            "Non valid keyword for use ! Must be AVBP, YALES2 or Cantera (default)"
        )
    ...
    if nTB + nFO > 0:
        pr.print_third_body(
            print_variables, constants, mech_variables, reactions_variables
        )
    if Plogindex:
        pr.print_pdep(
            print_variables,
    ...
    pr.print_reaction_rates(
        print_variables, constants, mech_variables, qss_variables, reactions_variables
    )
    if exponential:
        pr.print_production_rates_exp(
            print_variables,
            constants,
            mech_variables,
            qss_variables,
    ...
    if species_qss_names:
        qss.print_qss_concentrations(
            ...
            qss_variables,
            force=force,
            maintain_given_order=maintain_given_order,
        )
    ...
    if use == "Cantera":
        ...
        pr.print_avbp_specific(
            print_variables, mech_variables, reactions_variables, semi_implicit
        )
...
def compile_fortran(
    ...
    for lines in f.readlines():
        if (
            "subroutine" in lines.lower()
            and "(p,t,y,wdot)" in lines.replace(" ", "").lower()
        ):
            line_custom = lines
            break
    f.close()
    ...
    if line_custom == None:
        print(
            "The main subroutine of the f90 file should be written : 'subroutine customkinetics (P, T, y, wdot)' to work"
        )
        sys.exit()
    else:
    ...
    if not remote_install:
        ...
        if not os.path.isdir(path_to_dir):
            ...
            logger.info(
                "The necessary files for the compilation and customised kinetics run will be stored there"
            )
            logger.info(
                "If this location does not suit you, please specify a path as mechanism_lib argument"
            )
            logger.info(tail)
    ...
    if (
        not os.path.isfile(path_to_ext)
        or not filecmp.cmp(f90_filename, path_to_ext)
        or force
    ):
...
\tgfortran -shared -o customkinetics.so customkinetics.o
    """
            ...
            logger.info(
                "The correct Makefile has been created (" + path_to_makefile + ")"
            )
            # logger.info(tail)
...
That's it ! Enjoy using ARCANE."""
    ...
    if process != 0:
        logger.error(
            "The compilation of the f90 file failed ! There is something wrong ..."
        )
        if not output:
...
def cantera_qss_solution(
    ctmech, species_qss_names, qss_cti_file=None, transport=None, output_solution=True
):
    ...
    if species_objects_names != species_names:
        ...
        for spec in species_names:
            species_objects_sorted.append(
                species_objects[species_objects_names.index(spec)]
            )
    else:
    ...
    # Species non QSS
    species_non_qss = [
        s for s in species_objects_sorted if s.name not in species_qss_names
    ]
    ...
    if "H" not in ctmech.element_names:
        ...
        if (
            "O2" in species_non_qss_names
            and "CO2" in species_non_qss_names
            and "H2O" in species_non_qss_names
        ):
            nC = (
                ctmech.n_atoms(fuel, "C")
                if "C" in ctmech.species(fuel).composition
                else 0
            )
            nH = (
                ctmech.n_atoms(fuel, "H")
                if "H" in ctmech.species(fuel).composition
                else 0
            )
            nO = (
                ctmech.n_atoms(fuel, "O")
                if "O" in ctmech.species(fuel).composition
                else 0
            )
            ...
            if nC != 0 and "CO2" in species_non_qss_names:
                ...
                global_reaction = (
                    "reaction('" + fuel + " => " + fuel + "', [0.0, 0.0, 0.0])"
                )
            else:
                CO2string = ""
                global_reaction = (
...
def cantera_reduced_solution(
    ctmech,
    species_to_discard=[],
    reactions_to_discard=[],
    diluent=[],
    auto_remove=False,
    reduced_cti_file=None,
    transport=None,
):
    ...
    species_to_include = [
        s for s in species_objects_sorted if s.name not in species_to_discard
    ]
        ...
        if reac.equation in reactions_to_discard_equations:
            if reac.duplicate:
                ...
                if reac.reaction_type == 5:
                    ...
                    if (
                        reac.high_rate == reac_discard.high_rate
                        and reac.low_rate == reac_discard.low_rate
                    ):
                        continue
                else:
    ...
    automatically_removed = list(
        set(species_to_include_names) - set(species_in_a_reaction)
    )
    if automatically_removed and species_to_discard and auto_remove:
        logger.warning(
            "WARNING: Species "
            + ", ".join(automatically_removed)
            + " automatically removed since it was no longer present in a reaction"
        )
        species_to_include = [
            s for s in species_to_include if s.name in species_in_a_reaction
        ]
...
def reverse_arrhenius_fit(
    ...
    dx = (1.0 / end_temperature - 1.0 / start_temperature) / (
        number_of_temperatures - 1.0
    )
    ...
    reverse_Arrh, pcov = opt.curve_fit(
        getattr(sys.modules[__name__], f"calculate_lnk_{fit_order}"),
        temperatures,
        ln_kb,
    )
    ...
    if debug:
        ...
        lnb = getattr(sys.modules[__name__], f"calculate_lnk_{fit_order}")(
            temperatures, *reverse_Arrh
        )
...
def problematic_qss(mechanism, species_qss_list, remove_from="list"):
    ...
    if qss_not_found:
        logger.error(
            "Species "
            + str(", ").join(qss_not_found)
            + " is/are not in mechanism object"
        )  # + cti_filename)
        sys.exit()
    ...
    spec_in_thirdbody_expressions = list(
        set(species_qss_list).intersection(species_in_thirdbody_expressions)
    )
    ...
    for i in range(len(species_qss_list)):
        ...
        for j in index_coupled_qss_j[index_coupled_qss_i == i]:
            if j != i:
                for k in index_qss_reactions_j[index_qss_reactions_i == i]:
                    if (
                        nur[qss_index_global[i], k] >= 1
                        and nur[qss_index_global[j], k] >= 1
                    ) or (
                        (
                            nup[qss_index_global[i], k] >= 1
                            and nup[qss_index_global[j], k] >= 1
                        )
                        and is_reversible[k] == 1
                    ):
                        if k not in problematic_reactions:
                            problematic_reactions.append(k)
                            logger.debug(
                                "!!! WARNING !!! No quadratic coupling allowed (species "
                                + species_qss_list[i]
                                + " and "
                                + "species "
                                + species_qss_list[j]
        ...
        for k in index_qss_reactions_j[index_qss_reactions_i == i]:
            if nur[qss_index_global[i], k] > 1 or (
                nup[qss_index_global[i], k] > 1 and is_reversible[k] == 1
            ):
                if k not in problematic_reactions:
                    problematic_reactions.append(k)
                    logger.debug(
                        ...
                        + species_qss_list[i]
                        + " with coefficient "
                        + str(int(nur[qss_index_global[i], k]))
                        + " in reaction "
                        + str(k + 1)
                        + ")"
                    )
    ...
    if spec_in_thirdbody_expressions:
        logger.debug(
            "!!! WARNING !!! Invalid QSS species "
            + str(", ").join(spec_in_thirdbody_expressions)
            + " in thirdbody expressions"
        )
    ...
    if remove_from == "mechanism":
        ...
        problematic_reactions_objects = [
            reactions_objects[i] for i in problematic_reactions
        ]

        new_ctmech = cantera_reduced_solution(
            mechanism.ctmech, reactions_to_discard=problematic_reactions_objects
        )
        ...
        logger.debug(
            "Efficiencies have been updated so they no longer contain QSS species"
        )

        if problematic_reactions:
            problematic_reactions_names = [
                str(name + 1) for name in problematic_reactions
            ]
            logger.debug(
                "The given subset was not valid due to quadratic coupling"
                " and/or QSS species as specific collision partner"
            )
            logger.debug(
                "Implicated reactions and/or efficiencies "
                + ", ".join(problematic_reactions_names)
                + " have been removed"
            )
        ...
        while problematic_species:
            species_qss_names_proposition.remove(
                max(
                    problematic_species.keys(),
                    key=(lambda key: problematic_species[key]),
                )
            )
            problematic_species = {}
        ...
        if species_qss_names_proposition != species_qss_list:
            logger.debug(
                "The given subset was not valid due to quadtratic coupling "
                "and/or QSS species as specific collision partner"
            )
            logger.debug(
                "This coupling-free subset:"
                + str(species_qss_names_proposition)
                + "has been set to replace the previous one"
            )
