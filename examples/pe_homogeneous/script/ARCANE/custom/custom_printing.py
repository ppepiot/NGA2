...
def print_header(print_variables, use):
    ...
    if use in ["Cantera", "AVBP", "YALES2", "NTMIX", "NGA"]:
...
def print_declarations(
    ...
    text += (
        "  logical, parameter :: semiImplicit = ."
        + str(semi_implicit_bool)
        + "."
        + "\n"
    )
    text += "  \n"
    ...
    text += "  character(len=65), dimension(nreac + nreac_reverse) :: reacexp\n"
    text += "  \n"
    ...
    if use == "NGA":
        text += "  ! Properties of the species\n"
        text += "  real(WP), dimension(nspec) :: Cpsp,hsp\n"
    text += "  \n"
    ...
    if nTB > 0:
        ...
        for i in range(nTB + nTB_reverse):
            text += (
                "  integer, parameter :: TBr"
                + str(reac_label[ThreeBodiesindex[i]])
                + reac_direction[ThreeBodiesindex[i]]
                + " = "
                + str(i + 1)
                + "\n"
            )
    ...
    if nFO > 0:
        ...
        for i in range(nFO + nFO_reverse):
            text += (
                ...
                + str(reac_label[FallOffindex[i]])
                + reac_direction[FallOffindex[i]]
                + " = "
                + str(i + 1)
                + "\n"
            )
    ...
    if nPlog > 0:
        ...
        for i in range(len(Plogindex)):
            for index in range(Plog_pressures[reac_label[i] - 1]):
                text += (
                    "  integer, parameter :: Plogr"
                    + str(reac_label[Plogindex[i]])
                    + reac_direction[Plogindex[i]]
                    + str(index + 1)
                    + " = "
                    + str(k)
                    + "\n"
    ...
    for i in range(ns):
        if species[i] not in species_qss_names:
            text += (
                "\n"
                + "       "
                + str(tools.format_to_significant(MW[i] / 1000.0, 15))
                + "_"
                + precision
            )
            if count < ns - nqss - 1:
    ...
    if species_qss_names:
        for i in range(nqss):
            text += (
                ...
                + "      !"
                + str(
                    tools.format_to_significant(
                        MW[int(qss_index_global[i])] / 1000.0, 15
                    )
                )
                + "_"
                + precision
  ...
  subroutine reaction_expressions
    ...
    for i in range(nr + nr_reverse):
        if i >= nr:
            ...
            text += (
                "    reacexp(" + str(i + 1) + ") = " + '"' + str(reacname[i]) + '"\n'
            )
    text += "\n"
...
def print_pdep(
    print_variables, constants, arrhenius_constants, mech_variables, reactions_variables
):
...
"""
    ...
    for index_loc, index_glob in enumerate(Plogindex):
        for index in range(len(forward_arrh_pdep[reac_label[index_glob] - 1])):
            P = tools.format_to_significant(
                forward_arrh_pdep[reac_label[index_glob] - 1][index][3], 15
            )
            text += (
                "    P_pdep(Plogr"
                + str(reac_label[index_glob])
                + reac_direction[index_glob]
                + str(index + 1)
                + ") = "
                + str(P)
                + "_"
        ...
        for index in range(Plog_pressures[index_loc]):
            ...
            if index_glob < nr:
                log_A = tools.format_to_significant(
                    np.log(forward_arrh_pdep[reac_label[index_glob] - 1][index][0]), 15
                )
                beta = tools.format_to_significant(
                    forward_arrh_pdep[reac_label[index_glob] - 1][index][1], 15
                )
                Ea = tools.format_to_significant(
                    -forward_arrh_pdep[reac_label[index_glob] - 1][index][2], 15
                )
            else:
                log_A = tools.format_to_significant(
                    np.log(reverse_arrh_pdep[reac_label[index_glob] - 1][index][0]), 15
                )
                beta = tools.format_to_significant(
                    reverse_arrh_pdep[reac_label[index_glob] - 1][index][1], 15
                )
                Ea = tools.format_to_significant(
                    -reverse_arrh_pdep[reac_label[index_glob] - 1][index][2], 15
                )
...
def write_arrhenius(
    A, beta, Ea, alpha1=0, alpha2=0, alpha3=0, alpha4=0, precision="pr"
):
...
"""
    for i in range(nr + nr_reverse):
        if reac_type[i] == 5:
            ...
            A = tools.format_to_significant(
                rate_coefficients_array[0, reac_label[i] - 1], 15
            )
            beta = tools.format_to_significant(
                rate_coefficients_array[1, reac_label[i] - 1], 15
            )
            Ea = tools.format_to_significant(
                -rate_coefficients_array[2, reac_label[i] - 1], 15
            )
            if i < nr:
                alpha1 = "0.0"
                alpha2 = "0.0"
                ...
                alpha1 = tools.format_to_significant(
                    rate_coefficients_array[3, reac_label[i] - 1], 15
                )
                alpha2 = tools.format_to_significant(
                    rate_coefficients_array[4, reac_label[i] - 1], 15
                )
                alpha3 = tools.format_to_significant(
                    rate_coefficients_array[5, reac_label[i] - 1], 15
                )
                alpha4 = tools.format_to_significant(
                    rate_coefficients_array[6, reac_label[i] - 1], 15
                )

            text += f"    k(r{reac_label[i]}{reac_direction[i]}) = "
            text += write_arrhenius(
                A,
                beta,
                Ea,
                alpha1=alpha1,
            ...
            A = tools.format_to_significant(
                rate_coefficients_array[0, local_index_reac], 15
            )
            beta = tools.format_to_significant(
                rate_coefficients_array[1, local_index_reac], 15
            )
            Ea = tools.format_to_significant(
                -rate_coefficients_array[2, local_index_reac], 15
            )
            if i < nr:
                alpha1 = "0.0"
                alpha2 = "0.0"
                ...
                alpha1 = tools.format_to_significant(
                    rate_coefficients_array[3, local_index_reac], 15
                )
                alpha2 = tools.format_to_significant(
                    rate_coefficients_array[4, local_index_reac], 15
                )
                alpha3 = tools.format_to_significant(
                    rate_coefficients_array[5, local_index_reac], 15
                )
                alpha4 = tools.format_to_significant(
                    rate_coefficients_array[6, local_index_reac], 15
                )
            ...
            FallOffcoeff = [
                tools.format_to_significant(coeff, 15) for coeff in FallOffcoeff
            ]
            ...
            if mech.reaction(reac_label[i] - 1).falloff.type == "Simple":
                ...
                text += (
                    f"    FC(FOr{reac_label[i]}{reac_direction[i]}) = ((1.0_{precision} - {FallOffcoeff[0]}_{precision})*exp(-Tloc/({FallOffcoeff[1]}_{precision})) "
                ...
                text += (
                    f"    k(r{reac_label[i]}{reac_direction[i]}) = getlindratecoeff(Tloc, "
                ...
                logger.warning(
                    f"WARNING: The FallOff type {mech.reaction(reac_label[i] - 1).falloff.type} is not implemented yet, please contact a developer."
                )
...
"""
    ...
    if end_module_flag:
        end_module = "end module " + routine_name
    else:
        end_module = ""
...
def print_production_rates_exp(
    print_variables, constants, mech_variables, qss_variables, reactions_variables, use
):
...
"""
    ...
    if not implicit_exponential:
        ...
        text += (
            "    real("
            + precision
            + "), dimension(ne) :: largest_c, vector_X_solution, vector_B_in, B_diag  \n"
        )
            ...
            for spec_index in range(ns - nqss):
                text += (
                    ...
                    + str(
                        int(
                            n_atoms_in_spec(
                                species_og[spec_index], elements[element_index]
                            )
                        )
                    )
                    + "\n"
    ...
    text += (
        "    ! Compute concentrations at iteration n+1 following exponential method. tiny(1.0_"
        + precision
        + ")"
    ...
    for i in range(ns):
        if species[i] not in species_qss_names:
            if sum(nu_check[i, :]) != 0:
                ...
                text += ")"
                text += (
                    " / ( c(s" + str(species[i]) + ") + tiny(1.0_" + precision + "))"
                )  # - tiny(1.0_' + precision + ')' # -> if no clipping
                ...
                if use == "AVBP":
                    text += (
                        "    IF ( exp(coef_A(s"
                        + str(species[i])
                        + ") * dtchem_sub / thickness) == 1.0_pr ) THEN \n"
                    )
                    text += (
                        "      c_next(s"
                        + species[i]
                    ...
                    text += (
                        " * (exp(coef_A(s"
                        + str(species[i])
                        + ") * dtchem_sub / thickness) - 1.0_pr) & \n"
                        "      + c(s"
                        + str(species[i])
                        + ") * exp(coef_A(s"
                        + str(species[i])
                        + ") * dtchem_sub / thickness) \n"
                    )
                    text += "    END IF \n "
    ...
    if not implicit_exponential:
        ...
        text += (
            ...
            "The missing/excess atoms are splitted between " + str(ne) + " species \n"
        )
        text += (
            "    ! (as many as elements in the chemical system) with the largest local concentrations. "
        ...
        text += (
            "    ! Each time a species is selected, "
        ...
        text += (
            "    ! Build B. Add minus sign on the dAtom because X will be directly the number of atoms "
            "to compensate the loss/excess of it \n"
        )
        for e in range(ne):
            text += (
                "    vector_B_in(" + str(e + 1) + ") = - dAtom_" + elements[e] + "\n"
            )
        text += "   \n"
        ...
        text += (
            "    ! Compute matrix A determinant: if det=0, the matrix is not inversible "
            ...
            if i > 0:
                for j in range(i):
                    text += (
                        ...
                        + str(ne - i)
                        + ") - "
                        f"A_diag({str(ne - i)},{str(ne - j)}) * vector_X_solution({str(ne - j)}) \n"
                    )
                text += (
                    "      vector_X_solution("
                    + str(ne - i)
                ...
                text += (
                    ...
                    + str(ne - i)
                    + ") "
                    f"/ A_diag({str(ne - i)},{str(ne)}) \n"
                )
                text += "   \n"

        text += (
            "      ! The solutions are known: the concentrations correction to be applied is computed for the "
            + str(ne)
            + " concerned species \n"
        )
        for e in range(ne):
    ...
    text += (
        "    ! Finally the source terms are computed with exponential method "
    ...
    for i in range(ns):
        if species[i] not in species_qss_names:
            if sum(nu_check[i, :]) != 0:
                ...
                text += (
                    "    cdot(s" + str(species[i]) + ") = tiny(1.0_" + precision + ")"
                )
                text += "\n"
...
"""
    ...
    text = text.format(
        precision, routine_name, PLog_text, QSS_text, TB_text, production_text
    )

    f.write(text)


def print_avbp_specific(
    ...
    def print_semi_implicit(
        print_variables, mech_variables, reactions_variables, species_list
    ):
        ...
        if not timescales:
            logger.warning(
                "WARNING: The semi-implicitation needs reviewing, as the timescales "
                ...
                if nu[index_glob, myr] != 0:
                    ...
                    if nu[index_glob, myr] == 1:
                        ...
                        text += (
                            ...
                            + "\n"
                            + "         + "
                            + str(nu[index_glob, myr])
                            + "_{0} *"
                        )
                        flag = True
                    if flag:
                        text += (
                            " w(r" + str(reac_label[myr]) + reac_direction[myr] + ") "
                        )
                    if is_reversible[myr] == 1:
                        flag = False
                        if nu[index_glob, myr] != 0:
                            ...
                            if flag:
                                text += (
                                    " w(r"
                                    + str(reac_label[myr])
                                    + reac_direction[reverse_index[myr]]
                                    + ") "
                                )
            text += "\n\n"
...
"""
    ...
    def internal_avbp(print_variables, semi_implicit):
        ...
        if rrate:
            var_add = ", rrate"
            use_add = """\n    use input_param_defs, ONLY : nrrate_ARC, irrate_ARC
    use solut, ONLY : Lrrate_ARC
    use error, ONLY : print_error_and_quit"""
            declaration_add = (
                """
    real("""
                + str(precision)
                + """), dimension(nrrate_ARC) :: rrate
    integer :: i
...
"""
  ...
  subroutine internal_AVBP(P, T, thickness, c, wdot{5})
    ...
    use param_defs
    use misc_defs, ONLY: dtchem_sub{6}
    implicit none
...
"""
        ...
        if exponential:
            exp_text = (
                "    call get_production_rates_exp(wdot,w,c,dtchem_sub,thickness)"
            )
        else:
...
subroutine {1}(nnode,neqs,rhoinv,press,tempe,w_spec,wmol,source_spec{2})
  ...
  use param_defs
  use {1}, ONLY: internal_AVBP{3}
  use compute_defs, ONLY: thick
  implicit none
...
"""
        ...
        if rrate:
            var_add = ", rrate_ARC"
            use_add = "\n  use input_param_defs, ONLY : nrrate_ARC"
            declaration_add = (
                "\n  real("
                + str(precision)
                + "), dimension(1:nrrate_ARC,1:nnode), intent(OUT) :: rrate_ARC\n"
            )
            declaration_add_2 = (
                "\n  real("
                + str(precision)
                + "), dimension(1:nrrate_ARC) :: rrate_buf\n"
            )
            var_add_2 = ", rrate_buf"
            calc_add = (
                """
subroutine get_reacname_"""
                + str(routine_name)
                + """(reacexp_ARC)
  use input_param_defs, ONLY : nrrate_ARC, irrate_ARC
  use solut, ONLY : Lrrate_ARC
  use """
                + str(routine_name)
                + """, ONLY : reacexp, reaction_expressions
  implicit none
...
def print_nga_specific(
    ...
    """
    Prints NGA specific routines

    :param print_variables: dictionary containing the f90 file to write in and code specific infos
    :param constants: dictionary of constants needed for computation
    :param mech_variables: dictionary of mechanism related parameters needed for computation
    :param qss_variables: dictionary of qss related parameters needed for computation
    :param reactions_variables: dictionary of reactions related parameters needed for computation
    ...
    # NGA Variable declarations
    indexNonFalloff = len(ThreeBodiesindex)
    nreac = nr + nr_reverse

    nreacN = nreac - nTB - nTB_reverse - nFO - nFO_reverse
    nreacL = nreacN + indexNonFalloff
    nPrDepReac = len(FallOffindex)

    print(nreacN)
    print(nreacL)
    print(indexNonFalloff)
    print(nPrDepReac)
    ...
    #     print(r.equation,' ', mech.reaction_type(i))
    ...
    text = "! ===================== !\n"
    text += "! Cp and H computations !\n"
    text += "! ===================== !\n"
    text += "subroutine pdf_mech_thermodata(T)\n"
    text += "  use pdf_mech\n"
    text += "  implicit none\n\n"
    text += "  ! Input variable\n"
    text += "  real(WP) :: T\n\n"

    text += "  ! All species with default medium temperature of 1000K\n"
    text += "  if (T.gt.1000.00) then\n\n"
    MW = mech.molecular_weights / 1000.0

    for i, s in enumerate(mech.species()):
        coeffs = s.thermo.coeffs
        Tmed = coeffs[0]
        Ahi = coeffs[1:8]
        alo = coeffs[8:15]
        if Tmed == TmedD:
            text += (
                "    hsp(s"
                + s.name
                + ") = {:.12e}_WP".format(Rcst / MW[i])
                + " * ( T * (("
                + "{:.12e}".format(Ahi[0])
                + "_WP) + T * (({:.12e}".format(Ahi[1] / 2.0)
                + "_WP) + T * (({:.12e}".format(Ahi[2] / 3.0)
                + "_WP) + T * (({:.12e}".format(Ahi[3] / 4.0)
                + "_WP) + T * (({:.12e}".format(Ahi[4] / 5.0)
                + "_WP)))))) + ({:.12e}".format(Ahi[5])
                + "_WP))\n"
            )
            text += (
                "    Cpsp(s"
                + s.name
                + ") = "
                + "{:.12e}".format(Rcst / MW[i])
                + " * (("
                + "{:.12e}".format(Ahi[0])
                + "_WP) + T * (({:.12e}".format(Ahi[1])
                + "_WP)+ T * (({:.12e}".format(Ahi[2])
                + "_WP) + T * (({:.12e}".format(Ahi[3])
                + "_WP) + T * ({:.12e}".format(Ahi[4])
                + "_WP)))))\n\n"
            )
    text += "\n  else \n\n"
    for i, s in enumerate(mech.species()):
        ...
        if Tmed == TmedD:
            text += (
                ...
                + " * ( T * (("
                + "{:.12e}".format(alo[0])
                + "_WP) + T * (({:.12e}".format(alo[1] / 2.0)
                + "_WP) + T * (({:.12e}".format(alo[2] / 3.0)
                + "_WP) + T * (({:.12e}".format(alo[3] / 4.0)
                + "_WP) + T * (({:.12e}".format(alo[4] / 5.0)
                + "_WP)))))) + ({:.12e}".format(alo[5])
                + "_WP))\n"
            )
            text += (
                "    Cpsp(s"
                ...
                + " * (("
                + "{:.12e}".format(alo[0])
                + "_WP) + T * (({:.12e}".format(alo[1])
                + "_WP)+ T * (({:.12e}".format(alo[2])
                + "_WP) + T * (({:.12e}".format(alo[3])
                + "_WP) + T * ({:.12e}".format(alo[4])
                + "_WP)))))\n\n"
            )

    text += "  end if\n\n"
    text += "  ! Species with specific medium temperature\n"
    for i, s in enumerate(mech.species()):
        coeffs = s.thermo.coeffs
        Tmed = coeffs[0]
        Ahi = coeffs[1:8]
    ...
    text += "\n  return\n"
    text += "end subroutine pdf_mech_thermodata\n\n"
    text += "\n"

    text += "! ================= !\n"
    text += "! Export thermodata !\n"
    text += "! ================= !\n"
    text += "subroutine pdf_mech_get_thermodata(h,cp,T)\n"
    text += "  use pdf_mech\n"
    text += "  implicit none\n"
    text += "\n"
    text += "  real(WP), dimension(nspec) :: h,Cp\n"
    text += "  real(WP) :: T\n"
    text += "  \n"
    text += "  ! Calculate correct values from T\n"
    text += "  call pdf_mech_thermodata(T)\n"
    text += "\n"
    text += "  ! Export h and Cp arrays\n"
    text += "  h = hsp\n"
    text += "  Cp = Cpsp\n"
    text += "\n"
    text += "  return\n"
    text += "end subroutine pdf_mech_get_thermodata\n"
    text += "\n"

    # ------------------------------------------------ #
    # Derivative of Cp wrt T
    # ------------------------------------------------ #
    text += "! ===================== !\n"
    text += "! dCp/dT computation !\n"
    text += "! ===================== !\n"
    text += "subroutine pdf_mech_compute_dCpsdT(T,dCpsdT)\n"
    text += "  use pdf_mech\n\n"
    text += "  implicit none\n\n"
    text += "  real(WP), dimension(nspec), intent(out) :: dCpsdT\n"
    text += "  real(WP), intent(in) :: T\n\n"
    ...
    for i, s in enumerate(mech.species()):
        ...
        if Tmed == TmedD:
            text += (
                ...
                + ") = {:.12e}_WP".format(Rcst / MW[i])
                + " * (({:.12e}".format(Ahi[1])
                + "_WP) + T*(2.0_WP*({:.12e}".format(Ahi[2])
                + "_WP) + T*(3.0_WP*({:.12e}".format(Ahi[3])
                + "_WP) + T*4.0_WP*({:.12e}".format(Ahi[4])
                + "_WP))))\n"
            )
    ...
    for i, s in enumerate(mech.species()):
        ...
        if Tmed == TmedD:
            text += (
                ...
                + ") = {:.12e}_WP".format(Rcst / MW[i])
                + " * (({:.12e}".format(alo[1])
                + "_WP) + T*(2.0_WP*({:.12e}".format(alo[2])
                + "_WP) + T*(3.0_WP*({:.12e}".format(alo[3])
                + "_WP) + T*4.0_WP*({:.12e}".format(alo[4])
                + "_WP))))\n"
            )
    ...
    text += "\n  return\n"
    text += "end subroutine pdf_mech_compute_dCpsdT\n\n"

    # ------------------------------------------------ #
    # Species names
    # ------------------------------------------------ #
    text += "! ==================== !\n"
    text += "! Export species names !\n"
    text += "! ==================== !\n"
    text += "subroutine pdf_mech_get_speciesnames( names )\n"
    text += "  use pdf_mech\n"
    text += "  implicit none\n"
    text += "\n"
    text += "  character(len=str_medium), dimension(nspec) :: names\n"
    text += "\n"
    for i in range(ns):
        text += "  names(s" + species[i] + ") = '" + species[i] + "'\n"
    text += "\n"
    text += "  return\n"
    text += "end subroutine pdf_mech_get_speciesnames\n"
    text += "\n"
