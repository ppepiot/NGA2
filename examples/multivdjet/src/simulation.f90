...
contains
      ...
      do k=sc%cfg%kmino_,sc%cfg%kmaxo_
         do j=sc%cfg%jmino_,sc%cfg%jmaxo_
            do i=sc%cfg%imino_,sc%cfg%imaxo_
               sc%rho(i,j,k)=burke_schumann(sc%SC(i,j,k,1))
            end do
         end do
      end do
   ...
   subroutine simulation_init
      ...
      create_scalar: block
         ...
         sc=multivdscalar(cfg=cfg,scheme=quick,nscalar=1,name='MixFrac')
         ! Define boundary conditions
      ...
      allocate_work_arrays: block
         ...
         ! Scalar solver
         allocate(resSC(sc%cfg%imino_:sc%cfg%imaxo_,sc%cfg%jmino_:sc%cfg%jmaxo_,sc%cfg%kmino_:sc%cfg%kmaxo_,1:1))
      ...
      initialize_scalar: block
         ...
         do n=1,mybc%itr%no_
            ...
            sc%SC(i,j,k,1)=(Zcof+(Zjet-Zcof)*0.5_WP*(1.0_WP+tanh((0.5_WP*Djet-radius)/thick)))*0.5_WP*(1.0_WP+tanh((0.5_WP*Dcof-radius)/thick))
         end do
         ! Compute density
         call get_rho()
      ...
      create_ensight: block
         ...
         ens_out=ensight(cfg=cfg,name='multivdjet')
         ...
         call ens_out%add_scalar('mixfrac',sc%SC(:,:,:,1))
         ! Output to ensight
      ...
      create_monitor: block
         ...
         call mfile%add_column(sc%SCmax(1),'Zmax')
         call mfile%add_column(sc%SCmin(1),'Zmin')
         call mfile%add_column(sc%diffmax(1),'DIFFmax')
         call mfile%add_column(sc%diffmin(1),'DIFFmin')
         ...
         call consfile%add_column(sc%SCint(1),'SC integral')
         call consfile%add_column(sc%rhoint,'RHO integral')
         call consfile%add_column(sc%rhoSCint(1),'rhoSC integral')
         call consfile%write()
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         do while (time%it.le.time%itmax)
            ...
            ! Assemble explicit residual for each scalar
            do nsc=1,sc%nscalar
