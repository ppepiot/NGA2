...
module linprog
   ...
   integer, parameter :: iter_def=10         ! iter_max=iter_def*(nx+nc)
   real(WP)           :: tol_def=1.d-9       ! default tolerance
   real(WP)           :: tol                 ! tolerance
   real(WP)           :: tol_z_stop,tol_z_accept,tol_pivot,tol_neg,tol_aof      

   ! set lp_diag=0 for no diagnostic output; set lp_diag=1,2,3 for increasing o/p
   integer            :: lp_diag=0 

   ! if call_diag > 0,then o/p (with lp_diag=3) on call.eq.call_op only
   integer            :: call_diag=0 
   integer            :: lu=91
   logical            :: lu_set=.false.
...
contains
   ...
   subroutine lp(nx,nle,nge,neq,ale,age,aeq,ble,bge,beq,f,x,info,iter_lim,toler,check,feasible_only,stats,indic_pos,lhvars,t_z_stop,t_z_accept,t_pivot,t_neg,t_aof,lp_d,lu_d)
      ...
      real(WP), intent(in)  :: ale(nle,nx),age(nge,nx),aeq(neq,nx)  ! constraint matrices
      real(WP), intent(in)  :: ble(nle),bge(nge),beq(neq)           ! constraint vectors
      real(WP), intent(in)  :: f(nx)  ! objective function vector

      real(WP), intent(out) :: x(nx)  ! solution (or 0 if no solution)
      integer,  intent(out) :: info   ! status of solution

      integer,  intent(in),  optional :: iter_lim,lp_d,lu_d
      real(WP), intent(in),  optional :: toler
      real(WP), intent(in),  optional :: t_z_stop,t_z_accept,t_pivot,t_neg,t_aof
   ...
   subroutine lp_pos(tab,nc,nx,rhv,lhv,indic_pos)
      ...
      real(WP), intent(in)  :: tab(0:nc+1,0:nx)
   ...
   subroutine tab_op(calls,k,info,tab,nc,nx,rhv,lhv)
      ...
      real(WP), intent(in)    :: calls,tab(0:nc+1,0:nx)
   ...
   !> Simplex method
   subroutine lp_simplex(tab,nc,nx,nle,nge,neq,info,rhv,lhv,feas_only,iter_max,iter_feas,iter_opt)

      real(WP), intent(inout) :: tab(0:nc+1,0:nx)
   ...
   subroutine lp_feasible(tab,nc,nx,nle,nge,neq,rhv,lhv,col,ncol,info,iter_max,iter_feas)
   ...
   !> Returns index and value of max entry in row i over active columns col
   ...
   !> Returns index and value of entry with largest absolute value in row i over col
   ...
   subroutine lp_pivot(tab,nc,nx,ip,jp)
      ...
      do i=1,nc 
         ...
         else if (dxi.eq.dxmin) then  !  treat degeneracy: Bland's rule
            do j=1,nx  
   ...
   subroutine lp_exchange(tab,nc,nx,if_aux,ip,jp)
      ...
      integer      :: i
      real(WP) :: piv_inv,piv_col(0:nc+1),tab0
            
      piv_inv        =1.0_WP/tab(ip,jp)       ! inverse of pivot
      ...
      !  than the tolerance. 
      !  To guarantee non-negative left-hand variables,set tol_neg=0.0_WP  
   ...
   subroutine lp_check(nx,nle,nge,neq,ale,age,aeq,ble,bge,beq,x,err)
      ...
      real(WP), intent(in)  :: ale(nle,nx),age(nge,nx),aeq(neq,nx)  
      real(WP), intent(in)  :: ble(nle),bge(nge),beq(neq),x(nx)    
      real(WP), intent(out) :: err(5) 
      
      real(WP) :: zle(nle),zge(nge),zeq(neq)
